...
import org.firstinspires.ftc.teamcode.common.commandbase.commands.Intake.FullIntake;
...
public class SubsystemTest extends CommandOpMode {
    ...
    public void run() {
        ...
        telemetry.addData("Goofy:", robot.intakeSpinner.getCurrent(CurrentUnit.MILLIAMPS));

        if (robot.intakeSpinner.getCurrent(CurrentUnit.MILLIAMPS) > 4000 && robot.extendoMotor.getCurrentPosition() > Globals.EXTENDO_MAX_EXTENSION*0.5) {
            schedule(
        ...
        if (gamepad1.ps) {
            schedule(new ExtendAndSpinCommand(robot, Set.of(YELLOW, BLUE), Globals.EXTENDO_MAX_EXTENSION * 0.8));
        }
        ...
        if (gamepad1.right_trigger < .5 && root == 2) {
            ...
            schedule(
                    new ParallelCommandGroup(
                            new SpinnerCommand(robot.spinnerSubsystem, Globals.SpinnerState.STOPPED),
                            new DropdownCommand(robot.dropdownSubsystem, Globals.DropdownState.READY)
                    )
            );
        }
        if (gamepad1.triangle) {
            schedule(
                    new ParallelCommandGroup(
                            new DropdownCommand(robot.dropdownSubsystem, Globals.DropdownState.READY),
                            new ExtendoSlidesCommand(robot.extendoSubsystem, Globals.EXTENDO_MAX_EXTENSION*.8)
                    )
            );
        }
